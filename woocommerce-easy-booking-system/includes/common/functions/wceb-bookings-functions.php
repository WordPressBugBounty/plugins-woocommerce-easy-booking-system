...
function wceb_get_filtered_bookings( $filters = array() ) {
	...
	$bookings = ! empty( $query['placeholders'] ) ? $wpdb->get_results( $wpdb->prepare( $sql, $query['placeholders'] ) ) : $wpdb->get_results( $sql ); // phpcs:disable  WordPress.DB.PreparedSQL.NotPrepared
