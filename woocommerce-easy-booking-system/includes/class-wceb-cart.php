...
class Cart {
	...
	public function add_to_cart_validation( $passed, $product_id, $quantity, $variation_id = '', $variations = array() ) {
		...
		// phpcs:disable  WordPress.Security.NonceVerification.Recommended -- Already checked by WooCommerce.
		...
		$start = isset( $_REQUEST['start_date_submit'] ) ? wp_unslash( $_REQUEST['start_date_submit'] ) : false;  // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized in check_selected_dates().
		$end   = isset( $_REQUEST['end_date_submit'] ) ? wp_unslash( $_REQUEST['end_date_submit'] ) : false;  // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized in check_selected_dates().
		...
		// phpcs:enable  WordPress.Security.NonceVerification.Recommended -- Already checked by WooCommerce.
	...
	function add_cart_item_booking_data( $cart_item_meta, $product_id, $variation_id, $quantity ) {
		...
		try {
			...
			$booking_duration = Date_Selection_Helper::get_selected_booking_duration( $start, $end, $_product );
