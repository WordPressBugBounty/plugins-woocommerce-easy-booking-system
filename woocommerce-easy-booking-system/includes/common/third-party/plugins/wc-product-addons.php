...
function wceb_pao_save_multiply_option( $data, $i ) {
	...
	$multiply_addon = isset( $_POST['product_addon_multiply'] ) ? wp_unslash( $_POST['product_addon_multiply'] ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.NonceVerification.Missing -- nonce checked by WC Product Add-Ons
...
function wceb_pao_sanitize_selected_addons( $sanitized_data, $data ) {
	...
	if ( isset( $data['additional_cost'] ) ) {
		foreach ( $data['additional_cost'] as $i => $additional_cost ) {
...
function wceb_pao_addons_price_details( $details, $product, $data ) {
	...
	foreach ( $addons_data as $addon_data ) {
		...
		$details .= sprintf(
			esc_html__( '%1$s: %2$s', 'woocommerce-easy-booking-system' ),
