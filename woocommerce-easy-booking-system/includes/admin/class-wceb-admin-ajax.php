...
class Admin_Ajax {
	...
	public static function reports_product_search() {
		...
		$term = isset( $_GET['term'] ) ? (string) wc_clean( wp_unslash( $_GET['term'] ) ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized with wc_clean().
		...
		if ( $booked_product_ids ) {
			foreach ( $booked_product_ids as $product_id ) {
