...
class Order {
	 ...
	 **/
	public function hide_order_item_booking_data( array $item_meta ): array {
	...
	public function display_order_item_booking_data( $item_id, $item, $product ) {
		...
		if ( wceb_is_bookable( $product ) ) {
			...
			foreach ( $meta_array as $var => $meta_name ) {
				...
				${$var} = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT `meta_id` FROM $item_order_meta_table WHERE `order_item_id` = %d AND `meta_key` LIKE %s", // phpcs:ignore  WordPress.DB.PreparedSQL.InterpolatedNotPrepared
						$item_id,
						$meta_name
					)
				);
	...
	public function create_or_update_order_booking( $item ) {
		...
		// Prevent order refunds to trigger method.
		...
		// Order status not tracked by Easy Booking, delete booking.
		if ( ! in_array( 'wc-' . $order->get_status(), wceb_get_valid_order_statuses(), true ) ) {
