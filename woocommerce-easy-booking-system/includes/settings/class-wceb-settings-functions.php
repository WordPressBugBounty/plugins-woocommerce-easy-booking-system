			...
			<tbody class="<?php echo esc_attr( $args['body_classes'] ); ?>">
				...
				if ( $content ) {
					foreach ( $content as $item ) :
						echo self::table_columns( $columns, $args, $item ); // phpcs:ignore  WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped earlier
				endforeach;
				}
				?>
			</tbody>
			...
			<tfoot>
				<tr>
					<th colspan="<?php echo absint( $column_count ); ?>">
						<a href="#" class="button add-row add-<?php echo esc_attr( $args['content'] ); ?>" data-row="
						<?php
							ob_start();
							echo self::table_columns( $columns, $args ); // phpcs:ignore  WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped earlier
